...
class Appointment extends Connection
{
    ...
    {
        $sql = "SELECT a.*, CONCAT(b.fname, ' ', b.lname) AS docter, c.namaTreatment, c.hargaTreatment FROM appointment a JOIN (SELECT dentistID, fname, lname FROM dentist) AS b ON a.dentistID = b.dentistID JOIN (SELECT idTreatment, namaTreatment, hargaTreatment FROM treatment) AS c ON a.treatmentID = c.idTreatment WHERE a.appointmentID=$this->appointmentID";
        ...
        if (mysqli_num_rows($resultOne) == 1) {
            ...
            $this->docter = $data['docter'];
            $this->patientID = $data['patientID'];
            $this->date = $data['date'];
            $this->keluhan = $data['keluhan'];
            $this->time = $data['time'];
            $this->treatmentID = $data['treatmentID'];
            $this->namaTreatment = $data['namaTreatment'];
            $this->hargaTreatment = $data['hargaTreatment'];
